...
package org.elasticsearch.vpack.tenant.field;
...
public class IndexFieldMapper extends MetadataFieldMapper {
    ...
    static final class IndexFieldType extends MappedFieldType {
        ...
        public Query termsQuery(List values, QueryShardContext context) {
            ...
            String appname = context.getIndexSettings().getIndexMetaData().getSettings().get("index.tenant");
            for (Object value : values) {
                if (appname != null) {
                    value = rewrite(value, appname);
                }
                if (isSameIndex(value, context.getFullyQualifiedIndex().getName())) {
    ...
    private static <T> T maybeRewrite(T value, QueryShardContext context) {
        if (context == null) {
            return value;
        }
        String appname = context.getIndexSettings().getIndexMetaData().getSettings().get("index.tenant");
        return rewrite(value, appname);
    }

    private static <T> T rewrite(T value, String appname) {
        if (appname == null) {
            return value;
        }
        if (value instanceof BytesRef) {
            String newValue = IndexTenantRewriter.rewrite(((BytesRef) value).utf8ToString(), appname);
            value = (T) new BytesRef(newValue);
        }
        if (value instanceof String) {
            value = (T) IndexTenantRewriter.rewrite((String) value, appname);
        }
        return value;
    }
}
