...
public class MultiMatchQueryExtBuilder extends AbstractQueryBuilder<MultiMatchQueryExtBuilder> {
    public static final String NAME = "multi_match_plus";

    public static final MultiMatchQueryExtBuilder.Type DEFAULT_TYPE = MultiMatchQueryExtBuilder.Type.BEST_FIELDS;
    ...
    private final Map<String, Entry<Float, String>> fieldsBoosts;
    ...
    public enum Type implements Writeable {
        ...
        public static Type parse(String value, DeprecationHandler deprecationHandler) {
            MultiMatchQueryExtBuilder.Type[] values = MultiMatchQueryExtBuilder.Type.values();
            Type type = null;
            for (MultiMatchQueryExtBuilder.Type t : values) {
     ...
     */
    public MultiMatchQueryExtBuilder.Type getType() {
        return type;
    }
    ...
    public MultiMatchQueryExtBuilder(StreamInput in) throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            String similarity = in.readString();
            Entry<Float, String> boostSimilarity = new AbstractMap.SimpleEntry<>(boost, similarity);
            fieldsBoosts.put(field, boostSimilarity);
        }
    ...
    protected void doWriteTo(StreamOutput out) throws IOException {
        ...
        for (Entry<String, Entry<Float, String>> fieldsEntry : fieldsBoosts.entrySet()) {
            out.writeString(fieldsEntry.getKey());
            out.writeFloat(fieldsEntry.getValue().getKey());
            out.writeString(fieldsEntry.getValue().getValue());
        }
        type.writeTo(out);
     ...
     */
    public MultiMatchQueryExtBuilder field(String field) {
        if (Strings.isEmpty(field)) {
            throw new IllegalArgumentException("supplied field is null or empty.");
        }
        this.fieldsBoosts.put(field, new AbstractMap.SimpleEntry<>(AbstractQueryBuilder.DEFAULT_BOOST, null));
        return this;
    }
     ...
     */
    public MultiMatchQueryExtBuilder field(String field, float boost) {
        if (Strings.isEmpty(field)) {
            throw new IllegalArgumentException("supplied field is null or empty.");
        }
        this.fieldsBoosts.put(field, new AbstractMap.SimpleEntry<>(boost, null));
        return this;
    }
     ...
     */
    public MultiMatchQueryExtBuilder fields(Map<String, Map.Entry<Float, String>> fields) {
        this.fieldsBoosts.putAll(fields);
        return this;
    }

    public Map<String, Map.Entry<Float, String>> fields() {
        return fieldsBoosts;
    }
     ...
     */
    public MultiMatchQueryExtBuilder type(MultiMatchQueryExtBuilder.Type type) {
        if (type == null) {
     ...
     * Sets the type of the text query.
     */
    public MultiMatchQueryExtBuilder type(Object type) {
        if (type == null) {
            throw new IllegalArgumentException("[" + NAME + "] requires type to be non-null");
        }
     ...
     */
    public MultiMatchQueryExtBuilder operator(Operator operator) {
        if (operator == null) {
     ...
     */
    public MultiMatchQueryExtBuilder analyzer(String analyzer) {
     ...
     */
    public MultiMatchQueryExtBuilder slop(int slop) {
        if (slop < 0) {
     ...
     */
    public MultiMatchQueryExtBuilder fuzziness(Object fuzziness) {
        if (fuzziness != null) {
    ...
    public MultiMatchQueryExtBuilder prefixLength(int prefixLength) {
        if (prefixLength < 0) {
     ...
     */
    public MultiMatchQueryExtBuilder maxExpansions(int maxExpansions) {
        if (maxExpansions <= 0) {
    ...
    public MultiMatchQueryExtBuilder minimumShouldMatch(String minimumShouldMatch) {
    ...
    public MultiMatchQueryExtBuilder fuzzyRewrite(String fuzzyRewrite) {
     ...
     */
    @Deprecated
    public MultiMatchQueryExtBuilder useDisMax(Boolean useDisMax) {
    ...
    public MultiMatchQueryExtBuilder tieBreaker(float tieBreaker) {
        this.tieBreaker = tieBreaker;
        return this;
    }
    ...
    public MultiMatchQueryExtBuilder lenient(boolean lenient) {
    ...
    public MultiMatchQueryExtBuilder cutoffFrequency(float cutoff) {
    ...
    public MultiMatchQueryExtBuilder zeroTermsQuery(MatchQuery.ZeroTermsQuery zeroTermsQuery) {
    ...
    public MultiMatchQueryExtBuilder autoGenerateSynonymsPhraseQuery(boolean enable) {
     ...
     */
    public MultiMatchQueryExtBuilder fuzzyTranspositions(boolean fuzzyTranspositions) {
    ...
    public void doXContent(XContentBuilder builder, Params params) throws IOException {
        ...
        for (Entry<String, Entry<Float, String>> fieldEntry : this.fieldsBoosts.entrySet()) {
            builder.startObject();
            builder.field("field", fieldEntry.getKey());
            builder.field("boost", fieldEntry.getValue().getKey());
            builder.field("similarity", fieldEntry.getValue().getValue());
            builder.endObject();
        }
        builder.endArray();
    ...
    public static MultiMatchQueryExtBuilder fromXContent(XContentParser parser) throws IOException {
        Object value = null;
        Map<String, Entry<Float, String>> fieldsBoosts = new HashMap<>();
        MultiMatchQueryExtBuilder.Type type = DEFAULT_TYPE;
        ...
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if (QUERY_FIELD.match(currentFieldName, parser.getDeprecationHandler())) {
                    ...
                    type = MultiMatchQueryExtBuilder.Type.parse(parser.text(), parser.getDeprecationHandler());
        ...
        MultiMatchQueryExtBuilder builder = new MultiMatchQueryExtBuilder(value)
                .fields(fieldsBoosts)
                .type(type)
                .analyzer(analyzer)
    ...
    private static void parseFieldAndBoost(XContentParser parser, Map<String, Entry<Float, String>> fieldsBoosts) throws IOException {
        Map<String, Object> fieldAttrs = parser.map();
        String field = (String)fieldAttrs.get("field");
        Object boostObj = fieldAttrs.get("boost");
        float boost = boostObj instanceof Double? ((Double) boostObj).floatValue() :
                boostObj instanceof Float ? (Float)boostObj : 1f;
        String similarity = (String)fieldAttrs.get("similarity");
        if (fieldsBoosts.containsKey(field)) {
            throw new UnsupportedOperationException("unsupported multiple fields with same name(" + field + "), you can use combined similarity, such as: \"similarity\":\"BM25,DFR^3\"");
        }
        fieldsBoosts.put(field, new AbstractMap.SimpleEntry<>(boost, similarity));
    }
    ...
    protected Query doToQuery(QueryShardContext context) throws IOException {
        MultiMatchQueryExt multiMatchQuery = new MultiMatchQueryExt(context);
        if (analyzer != null) {
        ...
        Map<String, Map.Entry<Float, String>> newFieldsBoosts;
        if (fieldsBoosts.isEmpty()) {
            ...
            newFieldsBoosts = QueryParserHelperExt.resolveMappingFields(context, QueryParserHelperExt.parseFieldsAndWeights(defaultFields));
        } else {
            newFieldsBoosts = QueryParserHelperExt.resolveMappingFields(context, fieldsBoosts);
        }
    ...
    @Override
    protected boolean doEquals(MultiMatchQueryExtBuilder other) {
