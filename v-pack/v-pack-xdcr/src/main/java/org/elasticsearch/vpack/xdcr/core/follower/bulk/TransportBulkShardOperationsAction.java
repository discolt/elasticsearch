...
public class TransportBulkShardOperationsAction
    ...
    @Override
    protected TransportWriteAction.WritePrimaryResult<BulkShardOperationsRequest, BulkShardOperationsResponse> shardOperationOnPrimary(
...
//                        delete.versionType());
    ...
    public static XDCRWritePrimaryResult shardOperationOnPrimary(
        ...
        for (Translog.Operation sourceOp : sourceOperations) {
            long expectedSeqNo = primary.getLocalCheckpoint() + 1;
            long currentSeqNo = sourceOp.seqNo();
            if (expectedSeqNo != currentSeqNo) {
                String msg = "xdcr recovery failed [" + shardId + "] seqNo expected " + expectedSeqNo + " received " + currentSeqNo;
                throw new RuntimeException(msg);
            }

            // .getOperationPrimaryTerm()
            final Translog.Operation targetOp = rewriteOperationWithPrimaryTerm(sourceOp, primary.getPendingPrimaryTerm());
            ...
            if (result.getResultType() == Engine.Result.Type.SUCCESS) {
                ...
                if (result.getFailure() instanceof AlreadyProcessedFollowingEngineException) {
                    ...
                    logger.error("[xdcr] follower apply translogOperation failure", result.getFailure());
        ...
        final BulkShardOperationsRequest replicaRequest = new BulkShardOperationsRequest(
                shardId, appliedOperations);
        return new XDCRWritePrimaryResult(replicaRequest, location, primary, logger);
    }
     ...
     */
    static final class XDCRWritePrimaryResult extends TransportWriteAction.WritePrimaryResult<BulkShardOperationsRequest, BulkShardOperationsResponse> {
        private IndexShard primary;

        XDCRWritePrimaryResult(BulkShardOperationsRequest request, Translog.Location location, IndexShard primary, Logger logger) {
